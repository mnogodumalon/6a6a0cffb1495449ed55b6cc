...
export default function SachbearbeiterPage() {
  ...
  const [records, setRecords] = useState<Sachbearbeiter[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Sachbearbeiter | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Sachbearbeiter | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getSachbearbeiter());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Sachbearbeiter['fields']) {
  ...
  async function handleUpdate(fields: Sachbearbeiter['fields']) {
    ...
    await LivingAppsService.updateSachbearbeiterEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteSachbearbeiterEntry(deleteTarget.record_id);
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/sachbearbeiter/${record.record_id}`); }}>
