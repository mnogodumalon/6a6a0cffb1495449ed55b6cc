/**
 * SachbearbeiterDialog — pre-generated create/edit dialog for Sachbearbeiter.
 *
 ...
 * SachbearbeiterDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Sachbearbeiter['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<SachbearbeiterDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Sachbearbeiter';
...
/** Widened prefill type for SachbearbeiterDialog.defaultValues — see file header. */
export type SachbearbeiterDialogDefaults = Sachbearbeiter['fields'];

interface SachbearbeiterDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Sachbearbeiter['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: SachbearbeiterDialogDefaults;
...
export function SachbearbeiterDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: SachbearbeiterDialogProps) {
  const [fields, setFields] = useState<Partial<Sachbearbeiter['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['vorname', 'nachname', 'email'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Sachbearbeiter['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'sachbearbeiter');
      await onSubmit(clean as Sachbearbeiter['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Sachbearbeiter['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('sachbearbeiter') })
    : t('new_entity', { entity: appLabel('sachbearbeiter') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'vorname': (
      <div key="vorname" className="space-y-1.5">
        <Label htmlFor="vorname">{fieldLabel('sachbearbeiter', 'vorname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="vorname"
          placeholder=""
    ...
    'nachname': (
      <div key="nachname" className="space-y-1.5">
        <Label htmlFor="nachname">{fieldLabel('sachbearbeiter', 'nachname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="nachname"
          placeholder=""
    ...
    'email': (
      <div key="email" className="space-y-1.5">
        <Label htmlFor="email">{fieldLabel('sachbearbeiter', 'email')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="email"
          type="email"
          placeholder=""
    ...
    'telefon': (
      <div key="telefon" className="space-y-1.5">
        <Label htmlFor="telefon">{fieldLabel('sachbearbeiter', 'telefon')}</Label>
        <Input
          id="telefon"
    ...
    'abteilung': (
      <div key="abteilung" className="space-y-1.5">
        <Label htmlFor="abteilung">{fieldLabel('sachbearbeiter', 'abteilung')}</Label>
        <Input
          id="abteilung"
          placeholder=""
    ...
    'funktion': (
      <div key="funktion" className="space-y-1.5">
        <Label htmlFor="funktion">{fieldLabel('sachbearbeiter', 'funktion')}</Label>
        <Input
          id="funktion"
          placeholder=""
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "abteilung": "Abteilung", "funktion": "Funktion / Rolle"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.SACHBEARBEITER} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
