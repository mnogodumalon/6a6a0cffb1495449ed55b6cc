...
import type { Sachbearbeiter, Foerderantraege, CreateSachbearbeiter, CreateFoerderantraege } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async createSachbearbeiterEntry(fields: CreateSachbearbeiter): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.SACHBEARBEITER}/records`, { fields: cleanFieldsForApi(fields as any, 'sachbearbeiter') });
    return { ...data, record_id: data.id };
  }
  static async updateSachbearbeiterEntry(id: string, fields: Partial<CreateSachbearbeiter>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.SACHBEARBEITER}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'sachbearbeiter') });
    return { ...data, record_id: data.id };
  }
  static async deleteSachbearbeiterEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.SACHBEARBEITER}/records/${id}`);
  }
  ...
  static async createFoerderantraegeEntry(fields: CreateFoerderantraege): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.FOERDERANTRAEGE}/records`, { fields: cleanFieldsForApi(fields as any, 'foerderantraege') });
    return { ...data, record_id: data.id };
  }
  static async updateFoerderantraegeEntry(id: string, fields: Partial<CreateFoerderantraege>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.FOERDERANTRAEGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'foerderantraege') });
    return { ...data, record_id: data.id };
  }
  static async deleteFoerderantraegeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.FOERDERANTRAEGE}/records/${id}`);
  }
